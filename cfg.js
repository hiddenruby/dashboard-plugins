    ...
    let cfg =   setInterval(() =>{
                ...
                if ($('.settings_group.interface').length) { //create plug-ins section
                    if (!$('.dashboardPlugins-cfg').length){
                        ...
                        $('.dashboardPlugins-cfg .group_label').addClass('media-holder').append($('<span>').addClass('dashboardPlugins-tumblr-icon media-button media-killer').attr('id','user_findPlugins'));
                        ...
                        asyncForEach(Object.keys(plugins), function(pluginId){
                            if (!$('#user_enable_' + pluginId).length) {
                                ...
                                $('.dashboardPlugins-cfg .checkbox:last label:last').attr('for', 'user_enable_' + pluginId).text(plugin.name[locale()]).after($('<span>').addClass('dashboardPlugins-tumblr-icon media-button media-killer settings-icon-hollow').attr('id','user_configure_' + pluginId));
